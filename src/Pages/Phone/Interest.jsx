...
export const Interest = () => {
    ...
    useEffect(() => {
        const getBrand = async () => {
            setLoad(true)
            try {
                const response = await fetch(`https://phone-specs-clzpu7gyh-azharimm.vercel.app/top-by-interest`)
            ...
            <>
                ...
                <div className='flex flex-wrap justify-center gap-4 md:gap-3 lg:gap-14 lg:mx-4 mt-10'>
                    {getPhone.map((data,index) => 
                        <Card key={index} image={data.image} name={data.phone_name} brand={data.hits} slug={data.slug} />
                    )}
                </div>
